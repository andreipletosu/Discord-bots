...
def main():
    ...
    try:
      ...
      file1 = open("last_post.txt","r")
      last_post = file1.read()
      file1.close()
      if (last_post != get_last_publication_url(html)):
          time.sleep(float(os.environ.get('TIME_INTERVAL')))
          file = open("last_post.txt","w+")
          time.sleep(float(os.environ.get('TIME_INTERVAL')))
          file.write(get_last_publication_url(html))
          file.close()
          time.sleep(float(os.environ.get('TIME_INTERVAL')))
